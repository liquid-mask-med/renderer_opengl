...
static std::vector<glm::vec3> intersectSliceWithBox(const SliceDesc& slice, RenderBox box) {

	const glm::vec3& origin = slice.origin;
	const glm::vec3& axisU = slice.axisU;
	const glm::vec3& axisV = slice.axisV;

	const glm::vec3& normal = glm::normalize(glm::cross(axisU, axisV));

	const std::vector<Line2>& edges = box.getEdges();
	std::vector<glm::vec3> intersectedPoints;
	for (const auto& e : edges)
	{
		glm::vec3 l1 = e.p1 - origin;
		glm::vec3 l2 = e.p2 - origin;

		float da = dot(l1, normal);
		float db = dot(l2, normal);

		if (da * db <= 0) {

			if (abs(da - db) < std::numeric_limits<float>::epsilon()) {
				continue;
			}

			float t = da / (da - db);
			glm::vec3 p = e.p1 + t * (e.p2 - e.p1);
...
void Renderer::render(int mask)
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		if ((mask & (1 << i)) && viewW[i] > 0 && viewH[i] > 0) {
			...
			glUniform3fv(glGetUniformLocation(mprShader, "UVMin"), 1, glm::value_ptr(sliceUVBounds[i].min));
			glUniform3fv(glGetUniformLocation(mprShader, "UVMax"), 1, glm::value_ptr(sliceUVBounds[i].max));
...
void Renderer::updateSlice(int index, glm::vec3 origin, glm::vec3 axisU, glm::vec3 axisV)
{
	sliceStates[index] = { origin, axisU, axisV };

	const std::vector<glm::vec3> points = intersectSliceWithBox(sliceStates[index], renderBox);
	std::vector<glm::vec3> UVs;
	for (const auto& p : points)
	{
		glm::vec2 uv;
		float u = dot(p - sliceStates[index].origin, sliceStates[index].axisU);
		float v = dot(p - sliceStates[index].origin, sliceStates[index].axisV);
		UVs.push_back(glm::vec3(u, v, 0));
	}

	AABB boundary = AABB::generateAABB(UVs);
	sliceUVBounds[index] = boundary;
}
...
RENDERER_API void SetUpSliceState(Renderer* p, int index, Vec3 origin, Vec3 axisU, Vec3 axisV)
{
	...
	p->updateSlice(index, o, glm::normalize(u), glm::normalize(v));
}
